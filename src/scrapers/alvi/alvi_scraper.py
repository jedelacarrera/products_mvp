...
class AlviScraper(BaseScraper):
    ...
    def add_product(self, item):
        ...
        sale_info = (
            item.find_all("div", class_="sticker-value")[0].find("span").text.strip()
        )
        ...
        if len(splitted_sale) != 2:
            ...
            product = Product(
                name,
                description,
                "No valid price in product",
                "",
                "",
                "",
                image_url,
                "Error en este producto",
                "",
                item.text,
            )
        ...
        normal_price_info = item.find(
            "div", class_="card-footer text-center h8"
        ).span.text.strip()
